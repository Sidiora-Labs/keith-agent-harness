...
> spec/ (kvx). Per-IDE rule files are GENERATED pointers — never hand-edit; run Codify (cg) specgen to regenerate.
...
`spec/keith-self-evolution/spec.kvx` — read its `[meta]` status, `[req.*]` acceptance criteria, and the `[task.*]` list (status + wave + requires). Work one task at a time in wave order; update `status` in the kvx as you go.
...
- **Ui Component Library**: Client UI changes must follow the shared design system and existing component conventions.
